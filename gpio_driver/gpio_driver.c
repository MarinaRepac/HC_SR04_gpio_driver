...
MODULE_AUTHOR("Marina Repac, Aleksandar Zagorac");
...
volatile static int echo_width;
...
/* interrupt handler called when rising or falling edge on GPIO_17 (GPIO_ECHO) occurs */
...
{	
	...
	if (valid_value == 0) {
		...
		{
			echo_end = kt;
			echo_width = (int)ktime_to_us(ktime_sub(echo_end, echo_start));
			valid_value = 1;
		}
	}
...
int gpio_driver_init(void)
{
	...
	result = request_irq(irq_echo, h_irq_echo, IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING, "irq_echo", (void *)(h_irq_echo));
	if(result != 0)
    {
...
static ssize_t gpio_driver_read(struct file *filp, char *buf, size_t len, loff_t *f_pos)
{
	...
	/* Check if the object is out of range --> 400cm or more;
	   Check if the echo_width send to user is the current value */
	...
	while (valid_value == 0) 
	{
		...
		{
			echo_width = 0;		
			sprintf(gpio_driver_buffer, "%d", echo_width);
		}
		udelay(1);
	}
